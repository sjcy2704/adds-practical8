...
template <typename T> Heap<T>::Heap(std::vector<T> start_values) {
...
template <typename T> void Heap<T>::remove(T value) {
  int size = values.size();
  int parent = 0;
  for (int i = 0; i < size; i++) {
    if (value == values.at(i)) {
      values.at(i) = values.at(size - 1);
      values.pop_back();
      parent = floor((i - 1) / 2);
      break;
    }
...
template <typename T> T Heap<T>::getMin() { return values.at(0); }

/*******************************/
// private function to heapify a given 'node'
/*******************************/

template <typename T> void Heap<T>::heapify(int parent_index) {
